...
public class SendSmsServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try (Connection conn = DBConnection.DBconnection.getConnection()) {
            // Insert into sms table with "pending" status
            ...
            smsStmt.setString(7, "pending"); // Initial status is "pending"
            ...
            // Update status to "failed" and set the body to "Authentication failed"
            if (smsId != -1) {
                try (Connection conn = DBConnection.DBconnection.getConnection()) {
                    updateSmsStatus(conn, smsId, "failed", "Authentication failed");
    ...
    private void updateSmsStatus(Connection conn, int smsId, String status, String body) throws SQLException {
        String updateSql = "UPDATE sms SET status = ?, body = ? WHERE id = ?";
